...
if __name__ == "__main__":
	...
	context = {'policy_parameters':
				{'outcome_variable_name': 'fake_outcome', 'max_rating': 1, 'prior':{'success': 1, 'failure': 1}}, 
			'mooclet': 145, 'versions': [5197, 5198, 5199]}
	policy_id = 3 # id of your TS policy
	factor = 'fake' # The name of experimental factor. Only used to name the output files
	out_folder = './output/{}_offline/'.format(factor)
	csv_path = '{}{}_models_offline.csv'.format(out_folder, factor)
	stops = [10, 100, 790] # must not be larger than input TS data
	num_sims = 1000 # the number of simulations per data point in the input TS data
	levels = ['level 1', 'level 2', 'level 3'] # description of versions
	...
	data = data[data['policy_id_{}'.format(context['mooclet'])] == policy_id].sort_values('timestamp_{}'.format(context['mooclet']), ignore_index=True)
