...
from plot import plot_beta_dist, plot_assignment_prob, plot_success_failure
...
def get_arm(mooclet: int, user_id: str, token: str, mooclet_url: str, timeout=None):
	header = {'Authorization': 'Token ' + token}
	url = mooclet_url + '/engine/api/v1/mooclet/{}/run?user_id={}'.format(mooclet, user_id)
	return open_url_to_dict(url, None, 'GET', header, timeout)

def get_arm_parallel(mooclet: int, user_id: str, token: str, mooclet_url: str, versions: list, sim_count: int, wait_time: float, 
	...
	for _ in range(sim_count):
		...
		while arm == None:
			try:
				arm = get_arm(mooclet, user_id, token, mooclet_url)
...
def send_reward(variable: str, value: float, user_id: str, mooclet: int, version: int, policy:int, token: str, mooclet_url: str):
	url = mooclet_url + '/engine/api/v1/value'
...
if __name__ == "__main__":
	...
	sim_count = 1000 # the number of simulations per data point in the input TS data
	num_processes = 100 # must devide sim_count
	factor = 'finalstr7' # The name of experimental factor. Only used to name the output files
	...
	context = {'policy_parameters':
				{'outcome_variable_name': past_mooclet['outcome_variable_name'], 'max_rating': max_rating, 'prior':{'success': 1, 'failure': 1}}, 
	...
	data = data[data['policy_id_{}'.format(past_mooclet['mooclet'])] == policy_id].sort_values(
	...
	for i in range(data.shape[0]):
		...
		for _ in range(num_processes):
			p = multiprocessing.Process(target=get_arm_parallel, args=(
				sim_mooclet['mooclet'], 'learner', token, mooclet_url, sim_mooclet['versions'], sim_count // num_processes, wait_time, q))
			jobs.append(p)
			p.start()
		for proc in jobs:
			proc.join()
		...
		while response == None:
			try:
				response = send_reward(sim_mooclet['outcome_variable_name'], float(cur_user[past_mooclet['outcome_variable_name']]), 
					str(cur_user['StudentRandomID']), sim_mooclet['mooclet'], version, policy_id, token, mooclet_url)
			except (urllib.error.HTTPError, urllib.error.URLError) as e:
				print(str(e))
				sleep(wait_time)
